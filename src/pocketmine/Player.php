	...
	public function handleDataPacket(DataPacket $packet){
		...
		switch($packet::NETWORK_ID){
			case ProtocolInfo::LOGIN_PACKET:
				...
				if(strlen($packet->skin) !== Skin::SINGLE_SKIN_SIZE and strlen($packet->skin) !== Skin::DOUBLE_SKIN_SIZE){
					$this->close("", "disconnectionScreen.invalidSkin");
					break;
				}

				$skin = new Skin($packet->skin, $packet->skinId);
				$this->setSkin($skin);
			...
			case ProtocolInfo::REMOVE_BLOCK_PACKET:
				...
				$oldItem = clone $item;
				$particle = Server::getInstance()->getProperty("DestroyBlockParticle",true);
				if($this->canInteract($vector->add(0.5, 0.5, 0.5), $this->isCreative() ? 13 : 6) and $this->level->useBreakOn($vector, $item, $this,$particle)){
					if($this->isSurvival()){
						if(!$item->deepEquals($oldItem) or $item->getCount() !== $oldItem->getCount()){
							$this->inventory->setItemInHand($item);
							$this->inventory->sendHeldItem($this->hasSpawned);
