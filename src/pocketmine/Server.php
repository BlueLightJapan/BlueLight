...
class Server{
	 ...
	 * @return string
	 */
	public function getBlueLightVersion() : string{
		return \pocketmine\BLUELIGHT_VERSION;
	}
...
");
			...
			$this->logger->info(
				str_replace(
					$this->getName(),
					TextFormat::BLUE . $this->getName(),
					$this->getLanguage()->translateString("pocketmine.server.start", [" " . TextFormat::AQUA . $this->getVersion() . TextFormat::WHITE . " "])
				)
			);
			...
			$this->logger->info(TextFormat::GREEN . $poolSize . " AsyncWorkers has been activated.");// TODO: Lang
			...
			$this->logger->info($this->getLanguage()->translateString("pocketmine.server.info", [
				" " . $blue . $this->getName()  . $reset . TextFormat::WHITE . " ",
				($version->isDev() ? TextFormat::YELLOW : "") . $version->get(true) . TextFormat::WHITE . " ",
				$blue2 . $this->getCodename() . $reset . TextFormat::WHITE . " ",
				$this->getApiVersion()
			]));
