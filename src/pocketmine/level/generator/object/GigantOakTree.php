...
class GiantOakTree extends Tree{
	public $overridable = [
		Block::AIR => true,
		Block::LEAVES => true,
		Block::SAPLING => true
	...
	public function placeObject(ChunkManager $level, $x, $y, $z, Random $random){
		...
		$trunk = new VectorIterator($level, new Vector3($x + 1, $y -1, $z), new Vector3($x + 1, $y + $this->trunkHeight, $z));
		while($trunk->valid()){
			$trunk->next();
			$pos = $trunk->current();
			$level->setBlockIdAt($pos->x, $pos->y, $pos->z, Block::LOG);
		...
		$trunk = new VectorIterator($level, new Vector3($x + 1, $y -1, $z + 1), new Vector3($x + 1, $y + $this->trunkHeight, $z + 1));
		while($trunk->valid()){
			$trunk->next();
			$pos = $trunk->current();
			$level->setBlockIdAt($pos->x, $pos->y, $pos->z, Block::LOG);
		...
		$trunk = new VectorIterator($level, new Vector3($x, $y -1, $z + 1), new Vector3($x, $y + $this->trunkHeight, $z + 1));
		while($trunk->valid()){
			$trunk->next();
			$pos = $trunk->current();
			$level->setBlockIdAt($pos->x, $pos->y, $pos->z, Block::LOG);
			...
			for($zz = $z - $size; $zz <= $z + $size; $zz++){
				...
				if(($sizeX * $sizeX + $sizeZ * $sizeZ) <= ($size * $size)){
					if(isset($this->overridable[$level->getBlockIdAt($xx, $y, $zz)])){
						...
						$level->setBlockDataAt($xx, $y, $zz, 0);
					}
				}
			}
		}
