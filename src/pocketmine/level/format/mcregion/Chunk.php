...
class Chunk extends BaseFullChunk
{
    ...
    public function setBlockData($x, $y, $z, $data)
    {
        ...
        if (($y & 1) === 0) {
            $this->data{$i}
            = chr(($old_m & 0xf0) | ($data & 0x0f));
        } else {
            $this->data{$i}
            = chr((($data & 0x0f) << 4) | ($old_m & 0x0f));
        }
        $this->hasChanged = true;
    }
    ...
    public function setBlock($x, $y, $z, $blockId = null, $meta = null)
    {
        ...
        if ($meta !== null) {
            ...
            if (($y & 1) === 0) {
                $this->data{$i}
                = chr(($old_m & 0xf0) | ($meta & 0x0f));
                if (($old_m & 0x0f) !== $meta) {
                    $changed = true;
                }
            } else {
                $this->data{$i}
                = chr((($meta & 0x0f) << 4) | ($old_m & 0x0f));
    ...
    public function setBlockSkyLight($x, $y, $z, $level)
    {
        ...
        if (($y & 1) === 0) {
            $this->skyLight{$i}
            = chr(($old_sl & 0xf0) | ($level & 0x0f));
        } else {
            $this->skyLight{$i}
            = chr((($level & 0x0f) << 4) | ($old_sl & 0x0f));
        }
        $this->hasChanged = true;
    }
    ...
    public function setBlockLight($x, $y, $z, $level)
    {
        ...
        if (($y & 1) === 0) {
            $this->blockLight{$i}
            = chr(($old_l & 0xf0) | ($level & 0x0f));
        } else {
            $this->blockLight{$i}
            = chr((($level & 0x0f) << 4) | ($old_l & 0x0f));
        }
        $this->hasChanged = true;
    }
